...
PKGS3="phar|pgsql|redis|simplexml|soap|sockets|tokenizer|xml|xmlreader|xmlwriter|xdebug|zip"   && \
...
echo "############### [$(date)] Install Phar extension ###############"                        && \
printf "extension=phar.so\n" | tee /etc/php$PHPV0/conf.d/90-phar.ini                           && \
...
cd /usr/bin                                                                                    && \
rm -vrf /blackfire* /var/cache/apk/* /var/cache/distfiles/* /phphome \
    mysql_waitpid mysqlimport mysqlshow mysqladmin mysqlcheck mysqldump myisam_ftdump          && \
echo "###### Changing ownership of php log file directory ######"                              && \
chown -Rf 1971.php /var/log/php7                                                               && \
echo "################## [$(date)] Done ##################"                                    && \
